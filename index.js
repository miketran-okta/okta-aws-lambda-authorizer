...
exports.handler = function (event, context, callback) {
    ...
    jwtTokenHandler.verifyRequest({
        ...
        var options = {
            method: 'GET',
            url: 'https://dev-989484.oktapreview.com/api/v1/authorizationServers/ausfvgcz3zRxFrWkj0h7/policies/00pfvpyhtu75DlW7r0h7/rules',
            headers: {
                'Postman-Token': 'a33132de-9229-45fb-a0b6-97cf5d43a97e',
        ...
        request(options, function (error, response, body) {
            ...
            console.log('apiam policies body', body);

            var userRules = {
                "rules": []
            };
            var rulesResponse = JSON.parse(body);
            console.log('rulesResponse-1', rulesResponse);

            // pull all rules that this user is part of by username
            for (var ruleIndex = 0; ruleIndex < rulesResponse.length; ++ruleIndex) {

                // check by individual user
                var users = rulesResponse[ruleIndex].conditions.people.users.include;
                for (var userIndex = 0; userIndex < users.length; ++userIndex) {
                    if (users[userIndex] == claims.uid) {
                        console.log('user match found with: ', claims.uid);
                        userRules.rules.push(rulesResponse[ruleIndex]);
                    }

                }
            }
            
            console.log('rulesResponse-2', rulesResponse);

            var groupRequest = require("request");
            var options = {
                method: 'GET',
                url: 'https://dev-989484.oktapreview.com/api/v1/users/' + claims.uid + '/groups',
                headers: {
                    'Postman-Token': 'c1eadd08-45ca-43cc-a7bc-cd77fcfdbdcc',
                    'cache-control': 'no-cache',
                    Authorization: 'SSWS ' + process.env.API_KEY,
                    'Content-Type': 'application/json',
                    Accept: 'application/json'
            ...
            groupRequest(options, function (error, response, body) {
                if (error) throw new Error(error);

                var userGroups = JSON.parse(body);

                console.log('groupResponse', userGroups);
                console.log('rulesResponse-3', rulesResponse);

                // loop through all rules
                for (var ruleIndex = 0; ruleIndex < rulesResponse.length; ++ruleIndex) {
                    
                    // loop through all the groups in the policy
                    var policyGroups = rulesResponse[ruleIndex].conditions.people.groups.include;
                    for (var policyGroupIndex = 0; policyGroupIndex < policyGroups.length; ++policyGroupIndex) {
                                                
                        // loop through all the groups the user is a memberof
                        for(var userGroupsIndex = 0; userGroupsIndex < userGroups.length; ++userGroupsIndex) {
                            
                            // check if there is a match and add to userRules list
                            if(userGroups[userGroupsIndex].id == policyGroups[policyGroupIndex]) {
                                console.log('group match found with: ', policyGroups[policyGroupIndex]);
                                userRules.rules.push(rulesResponse[ruleIndex]);
                            }
                        }
                    }
                ...
                // loop through all rules which are applicable to the user
                for (var ruleIndex = 0; ruleIndex < userRules.rules.length; ++ruleIndex) {

                    var scopes = userRules.rules[ruleIndex].conditions.scopes.include;

                    // loop through the scopes for a rule
                    for (var scopeIndex = 0; scopeIndex < scopes.length; ++scopeIndex) {

                        var scope = method + resource;                        

                        if (scopes[scopeIndex] == scope) {
                            userScopes.scopes.push(scopes[scopeIndex]);
                        }

                    }
                }

                console.log('permitted scopes', userScopes.scopes.length);
                if (userScopes.scopes.length > 0) {
                    permit = true;
                }
        ...
        }); // end APIAM policy request

    }); // end JWT verifier request
}

function groupResonseCallback() {
